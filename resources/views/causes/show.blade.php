        ...
        <div class="row">
            <div class="col-md-8">
                ...
                <div class="mb-3">
                    ...
                    <strong>Raised:</strong> €{{ number_format($cause->totalDonations(),2) }} ({{ $cause->percentRaised() }}%)
                </div>
                    ...
                    @forelse($cause->donations as $donation)
                        <li class="list-group-item d-flex justify-content-between align-items-center">
                            <div>
                                ...
                                <div class="small text-muted">{{ $donation->date->format('M d, Y') }}</div>
                            </div>
            ...
            <div class="col-md-4">
                <div class="card p-3">
                    ...
                    <form method="POST" action="{{ route('causes.donations.store', $cause) }}">
                        @csrf
                        <div class="mb-2">
                            <input name="donor_name" class="form-control" placeholder="Your name" value="{{ old('donor_name') }}" required>
                            @error('donor_name')<div class="text-danger">{{ $message }}</div>@enderror
                        </div>
                        <div class="mb-2">
                            <input name="amount" type="number" step="0.01" class="form-control" placeholder="Amount (€)" value="{{ old('amount') }}" required>
